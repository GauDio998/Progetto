...
//Importo il modello dei prodotti dalla cartella models
...
//Richiesta GET su un singolo prodotto 
    ...
    .then(doc => {
        ...
        if (doc) {
            res.status(200).json({
                ...
                request: {
                    type: 'GET',
                    description: 'Per visualizzare tutti i prodotti:',
                    url: 'http://localhost:3000/products'
                } 
            });
        } else {
            res.status(404).json({message: 'Nessun dato trovato'});
        }
    })
    .catch(err => {
        console.log(err);
...
router.delete('/:productId', (req, res, next) => {
    ...
    .then(result => {
        res.status(200).json({
            message: 'Prodotto eliminato. Per aggiungere un prodotto seguire il seguente link',
            request: {
                type: 'POST',
                url: 'http://localhost:3000/products',
